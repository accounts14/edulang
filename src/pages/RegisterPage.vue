      ...
      <q-page class="flex flex-center bg-grey-2">
        <q-card class="auth-card row no-wrap shadow-24">
          ...
          <div class="col-md-5 gt-sm relative-position bg-primary overflow-hidden">
            <q-img src="https://images.unsplash.com/photo-1510070112810-d4e9a46d9e91?ixlib=rb-4.0.3" class="full-height" fit="cover" />
            <div class="absolute-center full-width text-center text-white q-pa-md" style="background: rgba(0,0,0,0.3); backdrop-filter: blur(2px);">
            ...
            <transition name="fade" mode="out-in">
              ...
              <div v-if="!isRegisterSuccess" key="form-register">
                <div class="column items-center q-mb-lg">
                  <q-avatar size="60px" color="blue-1" text-color="primary" icon="school" class="q-mb-md shadow-1" />
                ...
                <q-form @submit="handleRegister" class="q-gutter-y-md">
                  <div class="field-wrapper">
                    <label class="text-weight-bold text-caption q-ml-sm text-grey-9">Nama Lengkap</label>
                    <q-input v-model="form.name" placeholder="John Doe" filled rounded bg-color="grey-2" dense borderless class="q-mt-xs">
                      <template v-slot:append><q-icon name="person_outline" size="xs" color="grey-7" /></template>
                    </q-input>
                  </div>

                  <div class="field-wrapper">
                    <label class="text-weight-bold text-caption q-ml-sm text-grey-9">Email Address</label>
                    <q-input v-model="form.email" placeholder="example@email.com" filled rounded bg-color="grey-2" dense borderless class="q-mt-xs">
                      <template v-slot:append><q-icon name="mail_outline" size="xs" color="grey-7" /></template>
                    </q-input>
                  </div>

                  <div class="field-wrapper">
                    <label class="text-weight-bold text-caption q-ml-sm text-grey-9">Password</label>
                    <q-input v-model="form.password" :type="isPwd ? 'password' : 'text'" placeholder="••••••••" filled rounded bg-color="grey-2" dense borderless class="q-mt-xs">
                      <template v-slot:append>
                        <q-icon :name="isPwd ? 'visibility_off' : 'visibility'" size="xs" class="cursor-pointer" color="grey-7" @click="isPwd = !isPwd" />
                      </template>
                    </q-input>
                  </div>

                  <q-btn label="Continue" type="submit" color="primary" rounded unelevated class="full-width q-py-md text-weight-bold q-mt-lg shadow-3" :loading="loading" />
                  
                  <div class="text-center q-mt-lg text-body2">
                    Sudah punya akun? <span class="text-primary cursor-pointer text-weight-bold hover-underline" @click="$router.push('/login')">Masuk sekarang</span>
                  </div>
                </q-form>
              </div>

              <div v-else key="success-message" class="flex flex-center full-height column text-center animate-pop">
